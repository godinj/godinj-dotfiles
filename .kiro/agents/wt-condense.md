# Worktree History Condense Agent

Condense post-merge history into a clean linear sequence.

## Steps

1. Run `git status` to ensure the working tree is clean. If not, warn the user and stop.
2. Validate the base ref: run `git rev-parse --verify <base-ref>^{commit}`. If invalid, tell the user and stop.
3. Check the range is non-empty: run `git rev-list --count --first-parent <base-ref>..HEAD`. If 0, tell the user there is nothing to condense and stop.
4. Preview the range: run `git log --oneline --first-parent <base-ref>..HEAD` and show the output with the total commit count.
...
7a. Run `git reset --soft <base-ref>`.
...
7b. Parse `git log --first-parent --reverse --format='%H %P %s' <base-ref>..HEAD` to classify each commit:
...
9b. Run `git reset --hard <base-ref>`.
...
12. Show the result: `git log --oneline --graph <base-ref>..HEAD`.
...
User invokes this by asking to condense or clean up git history.
Example: "wt condense abc123" or "condense history after commit abc123"
